...
adCM2 <- buildSpatialGraph(adCM2, img_id = "ROI", type = "expansion", threshold = 20, coords = c("X_loc", "Y_loc"))
colPair(adCM2, "expansion_interaction_graph")

adNonCM <- buildSpatialGraph(adNonCM, img_id = "ROI", type = "expansion", threshold = 20, coords = c("X_loc", "Y_loc"))
colPair(adNonCM, "expansion_interaction_graph")
...
plotSpatial(adCM2, 
            ...
            directed = FALSE,
            edge_color_fix = "grey") + 
  scale_color_manual(values = adCM2@metadata[["pheno_cluster_colors"]]) +
...
plotSpatial(adNonCM, 
            ...
            directed = FALSE,
            edge_color_fix = "grey") + 
  scale_color_manual(values = adNonCM@metadata[["pheno_cluster_colors"]]) +
  ggtitle("expansion interaction graph")
...
# By celltypes
ad_CM <- aggregateNeighbors(ad_CM, colPairName = "expansion_interaction_graph", 
                         aggregate_by = "metadata", count_by = "pheno_cluster")

ad_nonCM <- aggregateNeighbors(ad_nonCM, colPairName = "expansion_interaction_graph", 
                            aggregate_by = "metadata", count_by = "pheno_cluster")

set.seed(220705)

cn_1 <- kmeans(ad_CM$aggregatedNeighbors, centers = 6)

cn_2 <- kmeans(ad_nonCM$aggregatedNeighbors, centers = 6)


ad_CM$cn_celltypes <- as.factor(cn_1$cluster)
ad_nonCM$cn_celltypes <- as.factor(cn_2$cluster)

col_CN <- setNames(colorRampPalette(brewer.pal(11, "Spectral"))(length(unique(ad_CM$cn_celltypes))), 
                   sort(unique(ad_CM$cn_celltypes)))

plotSpatial(ad_CM, 
            node_color_by = "cn_celltypes", 
            img_id = "ROI", 
            coords = c("X_loc", "Y_loc"),
            node_size_fix = 0.1) + scale_color_manual(values = col_CN)
  
#scale_color_brewer(palette = "Set3")

plotSpatial(ad_nonCM, 
            node_color_by = "cn_celltypes", 
            img_id = "ROI", 
            coords = c("X_loc", "Y_loc"),
            node_size_fix = 0.1) +
...
library(tidyverse)
for_plot <- colData(ad_CM) %>% as_tibble() %>%
...
library(tidyverse)
for_plot <- colData(ad_nonCM) %>% as_tibble() %>%
  group_by(cn_celltypes, pheno_cluster) %>%
  summarize(count = n()) %>%
  mutate(freq = count / sum(count)) %>%
  pivot_wider(id_cols = cn_celltypes, names_from = pheno_cluster, 
...
# Generate radius graph for SC detection (r=40) 
ad_CM <- buildSpatialGraph(ad_CM, img_id = "ROI", 
                        type = "expansion", 
                        name = "expansion_spatialcontext_graph", 
                        threshold = 40, coords = c("X_loc", "Y_loc"))

ad_nonCM <- buildSpatialGraph(ad_nonCM, img_id = "ROI", 
                           type = "expansion", 
                           name = "expansion_spatialcontext_graph", 
                           threshold = 40, coords = c("X_loc", "Y_loc"))

# Aggregate based on clustered_neighbors
ad_CM <- aggregateNeighbors(ad_CM, 
                         colPairName = "expansion_spatialcontext_graph",
                         aggregate_by = "metadata",
                         count_by = "cn_celltypes",
                         name = "aggregatedNeighborhood")

ad_nonCM <- aggregateNeighbors(ad_nonCM, 
                         colPairName = "expansion_spatialcontext_graph",
                         aggregate_by = "metadata",
                         count_by = "cn_celltypes",
                         name = "aggregatedNeighborhood")
...
ad_CM <- detectSpatialContext(ad_CM, 
                           entry = "aggregatedNeighborhood",
                           threshold = 0.90,
                           name = "spatial_context")

ad_nonCM <- detectSpatialContext(ad_nonCM, 
                           entry = "aggregatedNeighborhood",
                           threshold = 0.90,
                           name = "spatial_context")

# Define SC color scheme
col_SC <- setNames(colorRampPalette(brewer.pal(11, "Spectral"))(length(unique(ad_CM$spatial_context))), 
                   sort(unique(ad_CM$spatial_context)))
...
p1 <- plotSpatial(ad_CM[,ad_CM$ROI == "PM102_ROI1"], 
                  node_color_by = "cn_celltypes", 
                  img_id = "ROI", 
...
p2 <- plotSpatial(ad_CM[,ad_CM$ROI == "PM102_ROI1"], 
                  node_color_by = "spatial_context", 
                  img_id = "ROI", 
                  node_size_fix = 0.3, coords = c("X_loc", "Y_loc"),
                  colPairName = "expansion_spatialcontext_graph") +
...
ad_CM <- filterSpatialContext(ad_CM, 
                           entry = "spatial_context",
                           group_by = "ROI", 
                           group_threshold = 6,
                           cells_threshold = 1000)
...
ad_nonCM <- filterSpatialContext(ad_nonCM, 
                           entry = "spatial_context",
                           group_by = "ROI", 
                           group_threshold = 6,
                           cells_threshold = 1000)
...
ad <- patchDetection(ad, 
                     patch_cells = ad$pheno_cluster == "GC B cell",
                     img_id = "ROI",
                     expand_by = 1,
...
ad <- patchDetection(ad, 
                     patch_cells = ad$pheno_cluster == "Plasma cell",
                     img_id = "ROI",
                     expand_by = 1,
                     min_patch_size = 10, coords = c("X_loc", "Y_loc"),
                     colPairName = "expansion_interaction_graph")
...
#We can now calculate the fraction of Tfh cells within each GC B cell patch to roughly estimate cell infiltration.
library(tidyverse)
colData(ad) %>% as_tibble() %>%
  group_by(patch_id, Group) %>%
  summarize(Tcell_count = sum(pheno_cluster == "PD1+ Tfh CD4 T cell"),
            patch_size = n(),
            Tfh_cell_freq = Tcell_count / patch_size) %>%
  ggplot() +
  geom_point(aes(log10(patch_size), Tfh_cell_freq, color = Group)) +
  theme_classic()

#We can now calculate the fraction of B cells within each lymphoid patch to roughly estimate B cell infiltration.
colData(ad) %>% as_tibble() %>%
  group_by(patch_id, Group) %>%
  summarize(Bcell_count = sum(pheno_cluster == "GC B cell"),
            patch_size = n(),
...
ggplot(as.data.frame(patch_size)) + 
  ...
  geom_point(aes(Group, (size)))

p <- ggplot(as.data.frame(patch_size), aes(x=Group, y=size, fill=Group)) +
  geom_boxplot(outlier.alpha = 0.1, outlier.shape = 2)  + 
  geom_jitter(size = 4, shape=16, position=position_jitter(0.1)) +
  labs(y = "GC B cell patch size") +
...
df1_1 <- summarySE(as.data.frame(patch_size), measurevar="size", groupvars=c("Group"))
head(df1_1)

df1 <- data_summary(as.data.frame(patch_size), varname="size", groupnames=c("Group"))
head(df1)

compare_means(size ~ Group, data = as.data.frame(patch_size), method = "wilcox.test", p.adjust.method	= 'fdr')
...
library(scales)
out4 <- testInteractions(ad_nonCM, 
                        group_by = "ROI",
                        label = "pheno_cluster", 
                        colPairName = "expansion_interaction_graph",
                        BPPARAM = SerialParam(RNGseed = 221029))
...
saveRDS(out, "NE_directed_CM.rds")

out2 <- testInteractions(ad_nonCM, 
                        group_by = "ROI",
                        label = "pheno_cluster", 
                        colPairName = "expansion_interaction_graph",
                        method = "patch", 
                        patch_size = 3,
                        BPPARAM = SerialParam(RNGseed = 221029))
saveRDS(out2, "NE_directed_Non_CM.rds")
