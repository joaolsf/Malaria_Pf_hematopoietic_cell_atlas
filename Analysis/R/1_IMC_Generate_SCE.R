...
#1- Trying to use the Seurat method to convert anndata to h5seurat then seurat then SCE object.
library(anndata)
ad <- anndata::read_h5ad('adata_subset3.h5ad')
Convert('adata_subset3.h5ad', dest = "h5seurat", overwrite = FALSE, assay = "RNA")

#error when running LoadH5Seurat: https://github.com/mojaveazure/seurat-disk/issues/109
#Fix 1:
adata_subset3 <- LoadH5Seurat("adata_subset3.h5seurat", meta.data = FALSE, misc = FALSE) #lose some metadata

#2- Converting anndata to SingleCellExperiment object (SCE)
library(rhdf5)
library(zellkonverter)
ad <- readH5AD('adata_subset3.h5ad') #it worked and converted to SingleCell Experiment
