...
type_markers <- c('SMA', 'CD45','CD105',  'CD163','Iba1', 'CD68','CD14', 'CD16', 'CD56',
                   'CD11b','CD11c', 'CD19', 'CD20', 'CD3', 'CD4', 'CD8', 'CD235ab')

# Define cell_state_markers 
state_markers <- c("LAG3", "Foxp3", "CD45RA", "CD45RO", "IgD", 
                   "VISTA", "CD138", "VISTA", "CD27", "CX3CR1")
...
spe_cat$sample_id <- factor(spe$ROI)
spe_cat$Patient <- factor(spe$Patient)
spe_cat$condition <- factor(spe$Group)
spe_cat$cluster_id <- factor(spe$pheno_cluster)

#add celltype information to metadata
metadata(spe_cat)$cluster_codes <- data.frame(pheno_cluster = factor(spe_cat$pheno_cluster))
...
pbMDS(spe_cat, by = "cluster_id", assay = "X",
...
plotPbExprs(spe_cat, assay = "X", k = "pheno_cluster", features = rownames(spe_cat)[rowData(spe_cat)$marker_class == "type"],
